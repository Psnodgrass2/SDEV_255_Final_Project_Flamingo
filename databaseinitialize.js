...
db.serialize(() => {
  ...
  const insertCourse = db.prepare('INSERT INTO courses (name, description, creditHours, subjectArea, teacherId) VALUES (?, ?, ?, ?, ?)');

  courses.forEach((course) => {
    insertCourse.run(course.name, course.description, course.creditHours, course.subjectArea, course.teacherId);
  });
...
db.serialize(() => {
  ...
  const insertTeacher = db.prepare('INSERT OR REPLACE INTO teachers (id, name) VALUES (?, ?)');
